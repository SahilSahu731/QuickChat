    ...
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
        ...
        {messagesDummyData.map((msg, index) => (
            <div key={index} className={`flex items-end justify-end gap-2 ${msg.senderId !== "680f50e4f10f3cd28382ecf9" && "flex-row-reverse"}`}>
                {msg.image ? (
                    ...
                    <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all text-white bg-violet-500/30 ${msg.senderId === "680f50e4f10f3cd28382ecf9" ? "rounded-br-none" : "rounded-bl-none"}`}>{msg.text}</p>
                )}
      ...
      <div className="absolute flex bottom-0 left-0 right-0 items-center gap-3 p-3 ">
        <div className="flex-1 flex items-center px-3 rounded-full bg-gray-100/12">
            <input type="text" placeholder="Type a message" className="flex-1 p-3 border-none rounded-lg outline-none text-white placeholder-gray-400" />
